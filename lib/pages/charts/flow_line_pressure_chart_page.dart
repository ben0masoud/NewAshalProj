...
class FlowLinePressureChartPage extends StatelessWidget {
   FlowLinePressureChartPage({Key? key,required this.FLPSpots}) : super(key: key);


  List<FlSpot>? FLPSpots;
  @override
  ...
  Widget build(BuildContext context) {
    return
      Padding(
        ...
        child: (FLPSpots!.isNotEmpty) ? LineChart(
          LineChartData(
            lineBarsData: [
              LineChartBarData(
                  spots: FLPSpots, //points.map((point) => FlSpot(point.index.toDouble(), point.test)).toList(),
                  isCurved: false,
                  dotData: FlDotData(
                    show: true,
                  ),
